...
/**
 * CircleBrush.move: Generate the next location for paint.
 * @override:       Brush.move()
 * @param max:      Max step to next location
 * @optional neg:   Is the brush allowed to go off screen?
 */
CircleBrush.prototype.move = function (max, neg) {
    var newc = [];
    ...
    if (!neg) {
        if (newc[0] + this.radius >= this.ctx.canvas.clientWidth || newc[0] - this.radius <= 0) {
            newc[0] = this.coord[0];
        }

        if (newc[1] + this.radius >= this.ctx.canvas.clientHeight || newc[1] - this.radius <= 0) {
            newc[1] = this.coord[1];
        }
    }
...
/**
 * CircleBrush.smoothResize:    Generate a new radius based on previous width
...
/**
 * LinearBrush: Object for drawing lines
 * @param ctx:      Canvas context
 * @param coord:    Brush location ([x, y])
 * @param stroke:   Brush stroke size
 * @optional color: Stroke color
 */
function LinearBrush(ctx, coord, stroke, color) {
    Brush.call(this, ctx, coord, stroke, color);
    this.prev = coord;
}
LinearBrush.prototype = Object.create(Brush.prototype);
LinearBrush.prototype.constructor = LinearBrush;

/**
 * LinearBrush.move: Generate the next location for paint.
 * @override:       Brush.move()
 * @param max:      Max step to next location
 * @optional neg:   Is the brush allowed to go off screen?
 */
LinearBrush.prototype.move = function (max, neg) {
    var newc = [];
...
/**
 * LinearBrush.draw:    Draw a line between the previous coordinates and
 *                      the current ones
 */
LinearBrush.prototype.draw = function () {
    this.ctx.beginPath();    
    this.ctx.moveTo(this.prev[0], this.prev[1]);
    this.ctx.lineTo(this.coord[0], this.coord[1]);
    this.ctx.stroke();
    this.ctx.closePath();
}
